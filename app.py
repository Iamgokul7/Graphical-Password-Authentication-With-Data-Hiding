...
import io
import cv2
import mimetypes  # Add this with your other imports
import io     
from datetime import datetime  # For current_date    # Make sure this is also imported
    ...
    if request.method == 'POST':
        try:
            ...
            return render_template('file_upload.html',
                                success=f"File '{filename}' uploaded and encrypted successfully")
...
def view_file(file_id):
    ...
    try:
        # Get file info including extension
        access = db_execute(
        ...
        file_content = cipher.decrypt_and_verify(encrypted_data, tag)
        
        # Define which extensions can be displayed as text
        text_extensions = {'txt', 'csv', 'html', 'htm', 'xml', 'json', 'js', 'css', 'log','py'}
        
        if file_ext in text_extensions:
            try:
                decrypted_text = file_content.decode('utf-8')
        ...
        if file_ext in image_extensions:
            # Convert to base64 for display in img tag
        ...
        # For all other file types, offer download only
        return render_template('view_file.html',
                            filename=filename,
                            file_ext=file_ext,
                            file_id=file_id,
