...
def init_db():
    ...
    # Files table with file_ext column
    c.execute('''CREATE TABLE IF NOT EXISTS files
...
def file_upload():
    ...
    if request.method == 'POST':
        try:
            ...
            allowed_extensions = {'txt', 'png', 'jpg', 'jpeg', 'gif', 'csv', 'pdf', 'zip'}

            if '.' not in file.filename:
                return render_template('file_upload.html', error="Invalid file name")
            ...
            # Read file as binary for all types
            ...
            # Save with file extension
            conn = sqlite3.connect('users.db')
            c = conn.cursor()
            c.execute('''INSERT INTO files 
                      (owner_id, filename, file_ext, encrypted_data, key, nonce, tag) 
                      VALUES (?, ?, ?, ?, ?, ?, ?)''',
                      (session['user_id'], filename, file_ext, ciphertext, key, nonce, tag))
            conn.commit()
            conn.close()
...
from flask import Response, render_template, redirect, request, session
from Crypto.Cipher import AES
import base64
...
def view_file(file_id):
    ...
    try:
        ...
        access = db_execute(
            '''SELECT f.filename, f.file_ext, f.encrypted_data, f.key, f.nonce, f.tag
               FROM files f
               LEFT JOIN requests r ON f.id = r.file_id
        ...
        # Mark as viewed if requester (not owner)
        is_owner = db_execute(
        ...
        filename, file_ext, encrypted_data, key, nonce, tag = access
        ...
        if file_ext == 'txt':
            try:
                ...
                return redirect(f'/download_file/{file_id}')

        # Handle images
        elif file_ext in {'png', 'jpg', 'jpeg', 'gif'}:
            b64_content = base64.b64encode(decrypted_data).decode('utf-8')
            return render_template(
                'view_file.html',
                filename=filename,
        ...
        # Handle PDF files - shown inline in browser
        elif file_ext == 'pdf':
            return Response(
                decrypted_data,
...
@app.route('/test_download_link/<int:file_id>')
def test_download_link(file_id):
    return f"""
    <h1>Test Download Link</h1>
    <p>Generated URL: {url_for('download_file', file_id=file_id)}</p>
    <a href="{url_for('download_file', file_id=file_id)}">Test Download</a>
    """
...
#         mem_file = io.BytesIO(decrypted_data)
#         mem_file.seek(0)
...
#         response = make_response(decrypted_data) # type: ignore
...
def download_file(file_id):
    ...
    try:
        ...
        # Debug print to verify file data
        print(f"Found file: {filename} (type: {file_ext})")
